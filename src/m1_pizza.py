...
def run_test_generate_points_on_circle():
    ...
    # -------------------------------------------------------------------------
    expected = [rg.Point(275, 100),     # All numbers are approximate.
                rg.Point(225, 150),
                rg.Point(175, 100),
                rg.Point(225, 50.0)]
    circle = rg.Circle(rg.Point(225, 100), 50)
...
def run_test_draw_points_on_circle():
    ...
    # -------------------------------------------------------------------------
    # Test 4:
    title = 'DRAW_POINTS_ON_CIRCLE, test 4'
    window = rg.RoseWindow(500, 500, title)
    circle = rg.Circle(rg.Point(250, 250), 75)
    circle.fill_color = 'black'
    draw_points_on_circle(window, circle, 25, 'cyan')
    window.close_on_mouse_click()
...
def run_test_pizza():
    ...
    circle = rg.Circle(rg.Point(300, 350), 175)
    circle.fill_color = 'red'
    circle.outline_thickness = 5
    pizza(window, circle, 20, 'yellow', 2)
    window.close_on_mouse_click()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for k in range(len(points)):
        slices = rg.Line(points[k], circle.center)
        slices.color = color
...
def run_test_polygon():
    ...
    circle = rg.Circle(rg.Point(250, 150), 75)
    circle.outline_thickness = 3
...
def run_test_fancy_polygon():
    ...
    fancy_polygon(window, circle, 20, 5, 'red', 2)
    window.close_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    for k in range(len(points)):
        if k + hops_to_next_point < len(points):
            ...
            find_point = (k + hops_to_next_point) - len(points)
            line = rg.Line(points[k], points[find_point])
        line.thickness = thickness
        line.color = color
        line.attach_to(window)
    window.render()
